...
if [[ -z "$version" || -z "$build" ]]; then
  print -u2 "Usage: Tools/release.sh <marketing-version> <build-number> [--local|--notarized]"
  exit 64
fi

if [[ "$mode" != "--notarized" && "$mode" != "--local" ]]; then
  print -u2 "The optional third argument must be --local or --notarized."
  exit 64
fi
...
python3 Tools/validate_app_bundle.py "$app" \
  --version "$version" \
  --build "$build" \
  --require-universal \
